...
type NumMatrix struct {
	preSum [][]int // 表示从[0,0]到[i,j]的面积中的元素的总和
}

func Constructor1(matrix [][]int) NumMatrix { // Constructor
	rowLength, colLength := len(matrix)+1, len(matrix[0])+1 // 行0和列0只是作为边界使用，其实里面的元素都为0
	preSum := make([][]int, rowLength)
	preSum[0] = make([]int, colLength)
	for i := 1; i < rowLength; i++ {
		for j := 1; j < colLength; j++ {
			if preSum[i] == nil {
				preSum[i] = make([]int, colLength)
			}
