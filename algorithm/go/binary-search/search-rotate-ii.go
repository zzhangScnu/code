...
func searchRotateII(nums []int, target int) bool {
	l := len(nums)
	lo, hi, mi := 0, l-1, 0
	for lo <= hi {
		mi = lo + (hi-lo)>>1
		...
		if nums[lo] == nums[mi] && nums[mi] == nums[hi] { // 处理有重复数值时，无法判断哪边仍处于升序
			lo++
			hi--
		} else if nums[lo] <= nums[mi] { // 左侧数组仍是升序，注意此时判断索引已经不是0，而是lo（因为可能因重复而被处理过）
			// 目标值在左侧数组中
